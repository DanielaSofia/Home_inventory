...
- Ask: "Commit and push first?" → If yes: wait for user action, then re-run Step 1; if no: EXIT skill
- Do not proceed directly to Step 2 from this branch

**If unpushed commits:**
- Warn: "⚠️ N unpushed commits. CodeRabbit hasn't reviewed them"
- Ask: "Push now?" → If yes: `git push`, inform "CodeRabbit will review in ~5 min", EXIT skill; if no: EXIT skill
- On the next run after a successful push, re-run Step 1 before proceeding to Step 2
...
Assign `selected_pr_number` only from the user's explicit choice. If the user declines to choose among multiple matches, EXIT without using `pr_number`.
...
Check only the latest top-level CodeRabbit comment or review body for the in-progress message:
...
**If `review_in_progress` is `true`:** Inform "⏳ Review in progress, try again in a few minutes", EXIT
...
If a consolidated commit was created:
- Ask: "Push changes?" → If no: EXIT without posting a success summary
- If yes, record the consolidated commit SHA, run `git push`, and stop with an error if the push fails
- After a successful push, fetch the configured upstream and confirm with `git merge-base --is-ancestor` that the consolidated commit exists in the upstream ref
- Set `push_confirmed=true` only after that confirmation succeeds; otherwise EXIT without posting a success summary

If all deferred (no commit): Skip this step.

### Step 10: Post Summary

**If a consolidated commit was created and `push_confirmed == true`:** Post one success summary comment on the PR using the confirmed commit SHA:
